...
export async function prepareExportImage(source: string | Buffer, settings?: MainImageSettings) {
  ...
  if (!settings) return pipeline.png({ compressionLevel: 0 }).toBuffer();
  if (!settings.cutoutId && settings.trim) pipeline = pipeline.trim({ threshold: settings.trimThreshold });
  let data = await pipeline.png({ compressionLevel: 0 }).toBuffer();
  if (settings.cutoutId) {
    ...
    data = await sharp(data).extract({ left, top, width: right - left + 1, height: bottom - top + 1 }).png({ compressionLevel: 0 }).toBuffer();
  }
  if (settings.portrait) {
    const bounds = await sharp(data).metadata();
    if (bounds.width! > bounds.height! * 1.2) data = await sharp(data).rotate(90).png({ compressionLevel: 0 }).toBuffer();
  }
  if (settings.rotation) data = await sharp(data).rotate(settings.rotation, { background: settings.cutoutId ? { r: 0, g: 0, b: 0, alpha: 0 } : settings.background }).png({ compressionLevel: 0 }).toBuffer();
  if (settings.frame) {
